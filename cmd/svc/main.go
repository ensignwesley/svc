package main

import (
	"fmt"
	"os"
	"strings"
	"time"

	"github.com/ensignwesley/svc/internal/adder"
	"github.com/ensignwesley/svc/internal/checker"
	"github.com/ensignwesley/svc/internal/history"
	"github.com/ensignwesley/svc/internal/manifest"
	"github.com/ensignwesley/svc/internal/output"
	"github.com/ensignwesley/svc/internal/reporter"
	"github.com/ensignwesley/svc/internal/watcher"
)

const version = "1.5.0"

const initTemplate = `# services.yaml — Service Manifest
# Generated by: svc init
#
# This file is the source of truth for your self-hosted fleet.
#
#   svc status   — check live health for every service
#   svc check    — find drift between this file and what's running
#   svc add <id>         — scaffold a new entry from a running service
#   svc add --scan       — probe all operator-installed systemd units at once
#

manifest:
  version: 1       # required; schema version (currently only 1 supported)
  host: localhost  # default host for health checks; override per-service via health_url

  # Units to ignore during undocumented-unit scan (svc check).
  # Use for system-managed services you deliberately exclude from the manifest.
  # ignore_units:
  #   - openclaw-gateway.service

services:

  # ── Fully specified example ──────────────────────────────────────────────────
  my-service:
    # What this service does. One sentence. Required (not enforced, but expected).
    description: "My service — describe it here"

    # Port the service listens on.
    # Derives health URL as http://{host}:{port}/health
    port: 8080

    # Override the health endpoint URL (optional).
    # Required when port is absent (e.g. static sites, external services).
    # health_url: "https://example.com/healthz"

    # systemd unit name — adds systemctl is-active check alongside HTTP poll.
    # systemd_unit: "my-service.service"

    # GitHub owner/repo for version drift detection.
    # repo: "owner/repo"

    # Currently deployed version (semver). Required for version drift detection.
    # version: "1.0.0"

    # Pin to a major version track (e.g. 22 for Node.js LTS).
    # max_major: 1

    # Link to docs or source.
    # docs: "https://github.com/owner/repo"

    # Tags for filtering: svc status --tag <tag>
    # tags:
    #   - http
    #   - security

    # Date this entry was added. Auto-set by 'svc add'.
    # added: "2026-03-16"

  # ── Minimal example (static site, no local port) ────────────────────────────
  # my-static-site:
  #   description: "Static site served by nginx"
  #   health_url: "https://example.com/"
`

func main() {
	if len(os.Args) < 2 {
		printUsage()
		os.Exit(1)
	}

	cmd := os.Args[1]
	args := os.Args[2:]

	switch cmd {
	case "init":
		cmdInit(args)
	case "status":
		cmdStatus(args)
	case "check":
		cmdCheck(args)
	case "watch":
		cmdWatch(args)
	case "add":
		cmdAdd(args)
	case "history":
		cmdHistory(args)
	case "report":
		cmdReport(args)
	case "validate":
		cmdValidate(args)
	case "diff":
		cmdDiff(args)
	case "version", "--version", "-v":
		fmt.Printf("svc version %s\n", version)
	case "help", "--help", "-h":
		printUsage()
	default:
		fmt.Fprintf(os.Stderr, "svc: unknown command %q\nRun 'svc help' for usage.\n", cmd)
		os.Exit(1)
	}
}

// cmdInit scaffolds a services.yaml in the current directory.
func cmdInit(args []string) {
	outPath := "services.yaml"
	force := false

	for i := 0; i < len(args); i++ {
		switch args[i] {
		case "--out", "-o":
			if i+1 >= len(args) {
				fatal("--out requires a path argument")
			}
			i++
			outPath = args[i]
		case "--force":
			force = true
		default:
			fatalf("unknown flag: %s", args[i])
		}
	}

	if !force {
		if _, err := os.Stat(outPath); err == nil {
			fatalf("%s already exists — use --force to overwrite", outPath)
		}
	}

	if err := os.WriteFile(outPath, []byte(initTemplate), 0644); err != nil {
		fatalf("writing %s: %v", outPath, err)
	}

	fmt.Printf("Created %s\n", outPath)
	fmt.Println("Edit the file to describe your fleet, then run 'svc status'.")
}

// cmdStatus reads the manifest and polls all health endpoints.
func cmdStatus(args []string) {
	manifestPath := "services.yaml"
	filterTag := ""
	timeoutSec := 5
	jsonOut := false

	for i := 0; i < len(args); i++ {
		switch args[i] {
		case "--file", "-f":
			if i+1 >= len(args) {
				fatal("--file requires a path argument")
			}
			i++
			manifestPath = args[i]
		case "--tag":
			if i+1 >= len(args) {
				fatal("--tag requires a value")
			}
			i++
			filterTag = args[i]
		case "--timeout":
			if i+1 >= len(args) {
				fatal("--timeout requires a value")
			}
			i++
			fmt.Sscanf(args[i], "%d", &timeoutSec)
		case "--json":
			jsonOut = true
		default:
			fatalf("unknown flag: %s", args[i])
		}
	}

	m, err := manifest.LoadAuto(manifestPath)
	if err != nil {
		fatal(err.Error())
	}

	if len(m.Services) == 0 {
		fmt.Println("No services defined in manifest.")
		return
	}

	// Build targets map, applying tag filter.
	targets := make(map[string]string)
	for id, svc := range m.Services {
		if filterTag != "" && !hasTag(svc.Tags, filterTag) {
			continue
		}
		targets[id] = manifest.ResolveHealthURL(m, svc)
	}

	if len(targets) == 0 {
		fmt.Printf("No services match tag %q.\n", filterTag)
		return
	}

	results := checker.CheckAllHealth(targets, timeoutSec)

	if jsonOut {
		statuses := make([]output.ServiceStatus, 0, len(results))
		for _, r := range results {
			status := "down"
			if r.Up {
				status = "up"
			}
			statuses = append(statuses, output.ServiceStatus{
				ID:        r.ServiceID,
				Status:    status,
				LatencyMs: r.LatencyMS,
			})
		}
		out := output.StatusJSON{
			CheckedAt: output.Now(),
			Services:  statuses,
		}
		if err := output.WriteJSON(os.Stdout, out); err != nil {
			fatalf("json output: %v", err)
		}
		return
	}

	fmt.Printf("Checking %d service(s)...\n\n", len(targets))
	output.PrintStatusTable(os.Stdout, results)
}

func hasTag(tags []string, tag string) bool {
	for _, t := range tags {
		if t == tag {
			return true
		}
	}
	return false
}

// cmdAdd probes a running service and scaffolds a manifest entry to stdout.
func cmdAdd(args []string) {
	// Handle --scan mode first (no positional id required).
	for _, arg := range args {
		if arg == "--scan" {
			cmdAddScan(args)
			return
		}
	}

	if len(args) == 0 || strings.HasPrefix(args[0], "--") {
		fatal("svc add requires a service id\nUsage: svc add <id> [--port <n>] [--write]\n       svc add --scan [--write] [--include-known]")
	}

	id := args[0]
	portHint := 0
	write := false
	manifestPath := "services.yaml"
	timeoutSec := 5

	for i := 1; i < len(args); i++ {
		switch args[i] {
		case "--port", "-p":
			if i+1 >= len(args) {
				fatal("--port requires a value")
			}
			i++
			fmt.Sscanf(args[i], "%d", &portHint)
		case "--write":
			write = true
		case "--file", "-f":
			if i+1 >= len(args) {
				fatal("--file requires a path argument")
			}
			i++
			manifestPath = args[i]
		case "--timeout":
			if i+1 >= len(args) {
				fatal("--timeout requires a value")
			}
			i++
			fmt.Sscanf(args[i], "%d", &timeoutSec)
		default:
			fatalf("unknown flag: %s", args[i])
		}
	}

	fmt.Fprintf(os.Stderr, "Probing %q...\n", id)
	result := adder.Probe(id, portHint, timeoutSec)

	today := time.Now().UTC().Format("2006-01-02")

	yaml := adder.Scaffold(result, today)

	if !write {
		fmt.Println("# Add the following to your services.yaml:")
		fmt.Println()
		fmt.Print(yaml)
		fmt.Println()
		fmt.Fprintf(os.Stderr, "Review the entry, then run: svc add %s --write\n", id)
		return
	}

	// --write: append to manifest file.
	f, err := os.OpenFile(manifestPath, os.O_APPEND|os.O_WRONLY, 0644)
	if err != nil {
		fatalf("opening %s: %v", manifestPath, err)
	}
	defer f.Close()
	if _, err := fmt.Fprint(f, "\n"+yaml); err != nil {
		fatalf("writing to %s: %v", manifestPath, err)
	}
	fmt.Fprintf(os.Stderr, "Added %q to %s\n", id, manifestPath)
}

// cmdAddScan discovers all operator-installed systemd units and scaffolds
// manifest entries for any not already documented.
func cmdAddScan(args []string) {
	write := false
	includeKnown := false
	manifestPath := "services.yaml"
	timeoutSec := 5

	for i := 0; i < len(args); i++ {
		switch args[i] {
		case "--scan":
			// already handled — skip
		case "--write":
			write = true
		case "--include-known":
			includeKnown = true
		case "--file", "-f":
			if i+1 >= len(args) {
				fatal("--file requires a path argument")
			}
			i++
			manifestPath = args[i]
		case "--timeout":
			if i+1 >= len(args) {
				fatal("--timeout requires a value")
			}
			i++
			fmt.Sscanf(args[i], "%d", &timeoutSec)
		default:
			fatalf("unknown flag: %s", args[i])
		}
	}

	// Load existing manifest to build known-ID set.
	knownIDs := make(map[string]bool)
	if m, err := manifest.LoadAuto(manifestPath); err == nil {
		for id := range m.Services {
			knownIDs[id] = true
		}
	}

	fmt.Fprintln(os.Stderr, "Scanning operator-installed systemd units...")
	results, err := adder.ScanFleet(knownIDs, timeoutSec)
	if err != nil {
		fatalf("scan failed: %v", err)
	}

	if len(results) == 0 {
		fmt.Fprintln(os.Stderr, "No operator-installed units found.")
		return
	}

	today := time.Now().UTC().Format("2006-01-02")

	var toWrite []adder.ScanResult
	for _, r := range results {
		if r.AlreadyInManifest && !includeKnown {
			fmt.Fprintf(os.Stderr, "  skip  %s (already in manifest)\n", r.ID)
			continue
		}
		toWrite = append(toWrite, r)
	}

	if len(toWrite) == 0 {
		fmt.Fprintln(os.Stderr, "All discovered units are already in the manifest.")
		fmt.Fprintln(os.Stderr, "Use --include-known to scaffold entries for existing services too.")
		return
	}

	// Build combined scaffold output.
	var combined strings.Builder
	for _, r := range toWrite {
		label := ""
		if r.AlreadyInManifest {
			label = " (already documented)"
		}
		fmt.Fprintf(os.Stderr, "  probe %s%s\n", r.ID, label)
		combined.WriteString(adder.Scaffold(r.ProbeResult, today))
	}

	if !write {
		fmt.Printf("# svc add --scan — %d unit(s) found, %d new\n", len(results), len(toWrite))
		fmt.Printf("# Add the following to your services.yaml:\n\n")
		fmt.Print(combined.String())
		fmt.Fprintf(os.Stderr, "\nReview the entries, then run: svc add --scan --write\n")
		return
	}

	// --write: append all new entries to manifest.
	f, err := os.OpenFile(manifestPath, os.O_APPEND|os.O_WRONLY, 0644)
	if err != nil {
		fatalf("opening %s: %v", manifestPath, err)
	}
	defer f.Close()
	if _, err := fmt.Fprint(f, "\n"+combined.String()); err != nil {
		fatalf("writing to %s: %v", manifestPath, err)
	}
	fmt.Fprintf(os.Stderr, "Added %d service(s) to %s\n", len(toWrite), manifestPath)
}

// cmdWatch runs the continuous polling loop with state-change alerting.
func cmdWatch(args []string) {
	cfg := watcher.Config{
		ManifestPath:    "services.yaml",
		StatePath:       watcher.DefaultStatePath(),
		DeliveryLogPath: watcher.DefaultDeliveryFailurePath(),
		Interval:        60,
		FailThreshold:   2,
		TimeoutSec:      5,
	}

	for i := 0; i < len(args); i++ {
		switch args[i] {
		case "--file", "-f":
			if i+1 >= len(args) {
				fatal("--file requires a path argument")
			}
			i++
			cfg.ManifestPath = args[i]
		case "--webhook":
			if i+1 >= len(args) {
				fatal("--webhook requires a URL")
			}
			i++
			cfg.WebhookURL = args[i]
		case "--interval":
			if i+1 >= len(args) {
				fatal("--interval requires a value")
			}
			i++
			fmt.Sscanf(args[i], "%d", &cfg.Interval)
		case "--failures":
			if i+1 >= len(args) {
				fatal("--failures requires a value")
			}
			i++
			fmt.Sscanf(args[i], "%d", &cfg.FailThreshold)
		case "--state":
			if i+1 >= len(args) {
				fatal("--state requires a path")
			}
			i++
			cfg.StatePath = args[i]
		case "--timeout":
			if i+1 >= len(args) {
				fatal("--timeout requires a value")
			}
			i++
			fmt.Sscanf(args[i], "%d", &cfg.TimeoutSec)
		case "--no-systemd":
			cfg.NoSystemd = true
		case "--stdout":
			cfg.Stdout = true
		default:
			fatalf("unknown flag: %s", args[i])
		}
	}

	if err := watcher.Watch(cfg, os.Stdout); err != nil {
		fatal(err.Error())
	}
}

// cmdCheck diffs the manifest against what's actually running.
func cmdCheck(args []string) {
	manifestPath := "services.yaml"
	noVersion := false
	noSystemd := false
	timeoutSec := 5
	jsonOut := false
	record := false
	historyPath := history.DefaultPath()

	for i := 0; i < len(args); i++ {
		switch args[i] {
		case "--file", "-f":
			if i+1 >= len(args) {
				fatal("--file requires a path argument")
			}
			i++
			manifestPath = args[i]
		case "--no-version":
			noVersion = true
		case "--no-systemd":
			noSystemd = true
		case "--timeout":
			if i+1 >= len(args) {
				fatal("--timeout requires a value")
			}
			i++
			fmt.Sscanf(args[i], "%d", &timeoutSec)
		case "--json":
			jsonOut = true
		case "--record":
			record = true
		case "--history-file":
			if i+1 >= len(args) {
				fatal("--history-file requires a path")
			}
			i++
			historyPath = args[i]
		default:
			fatalf("unknown flag: %s", args[i])
		}
	}

	m, err := manifest.LoadAuto(manifestPath)
	if err != nil {
		fatal(err.Error())
	}

	if len(m.Services) == 0 {
		fmt.Println("No services defined in manifest.")
		return
	}

	// Build ignore set from manifest.
	ignoreSet := make(map[string]bool)
	for _, u := range m.Meta.IgnoreUnits {
		ignoreSet[u] = true
	}
	// Also ignore units declared in the manifest itself.
	manifestUnits := make(map[string]bool)
	for _, svc := range m.Services {
		if svc.SystemdUnit != "" {
			manifestUnits[svc.SystemdUnit] = true
		}
	}

	// 1. Health checks.
	targets := make(map[string]string)
	for id, svc := range m.Services {
		targets[id] = manifest.ResolveHealthURL(m, svc)
	}
	healthResults := checker.CheckAllHealth(targets, timeoutSec)

	// 2. Systemd checks.
	systemdResults := make(map[string]checker.SystemdResult)
	useSystemd := !noSystemd && checker.SystemdAvailable()
	if useSystemd {
		for id, svc := range m.Services {
			if svc.SystemdUnit != "" {
				if checker.IsRemoteHost(svc.Host) {
					r := checker.CheckRemoteUnit(svc.Host, svc.SystemdUnit, timeoutSec)
					systemdResults[id] = checker.SystemdResult{
						Unit:      r.Unit,
						Active:    r.Active,
						LoadState: r.LoadState,
						Err:       r.Err,
					}
				} else {
					systemdResults[id] = checker.CheckUnit(svc.SystemdUnit)
				}
			}
		}
	}

	// 3. Version checks (stub for now — will add in v0.2).
	_ = noVersion

	// 3b. Record to history if --record is set.
	if record {
		hdb, err := history.Open(historyPath)
		if err != nil {
			fmt.Fprintf(os.Stderr, "⚠️  history: could not open db: %v\n", err)
		} else {
			defer hdb.Close()
			now := time.Now().UTC()
			for _, r := range healthResults {
				svc := m.Services[r.ServiceID]
				host := svc.Host
				if host == "" {
					host = "localhost"
				}
				status := "up"
				var latPtr *int64
				var errPtr *string
				if r.Up {
					lat := r.LatencyMS
					latPtr = &lat
				} else {
					status = "down"
					if r.Err != "" {
						errPtr = &r.Err
					}
				}
				_ = hdb.Record(history.CheckRow{
					ServiceID: r.ServiceID,
					Host:      host,
					CheckedAt: now,
					Status:    status,
					LatencyMS: latPtr,
					Error:     errPtr,
				})
			}

			// Auto-prune if manifest.history.retention is set.
			if retDur, err := manifest.RetentionDuration(m); err == nil && retDur > 0 {
				keepSecs := int64(retDur / time.Second)
				if _, pruneErr := hdb.Prune(keepSecs); pruneErr != nil {
					fmt.Fprintf(os.Stderr, "⚠️  history: auto-prune failed: %v\n", pruneErr)
				}
			}
		}
	}

	// 4. Undocumented units scan.
	var undocumented []string
	if useSystemd {
		operatorUnits, err := checker.ListOperatorUnits()
		if err == nil {
			for _, unit := range operatorUnits {
				if !manifestUnits[unit] && !ignoreSet[unit] {
					undocumented = append(undocumented, unit)
				}
			}
		}
	}

	if jsonOut {
		statuses := make([]output.ServiceStatus, 0, len(healthResults))
		for _, r := range healthResults {
			status := "down"
			if r.Up {
				status = "up"
			}
			ss := output.ServiceStatus{
				ID:        r.ServiceID,
				Status:    status,
				LatencyMs: r.LatencyMS,
			}
			if sr, ok := systemdResults[r.ServiceID]; ok {
				active := sr.Active
				ss.SystemdActive = &active
			}
			statuses = append(statuses, ss)
		}
		// Calculate drift for JSON output.
		driftJSON := 0
		for _, r := range healthResults {
			if !r.Up {
				driftJSON++
			}
		}
		for _, sr := range systemdResults {
			if !sr.Active {
				driftJSON++
			}
		}
		driftJSON += len(undocumented)
		exitCode := 0
		if driftJSON > 0 {
			exitCode = 1
		}
		out := output.CheckJSON{
			CheckedAt:    output.Now(),
			Services:     statuses,
			Undocumented: undocumented,
			DriftCount:   driftJSON,
			ExitCode:     exitCode,
		}
		if err := output.WriteJSON(os.Stdout, out); err != nil {
			fatalf("json output: %v", err)
		}
		os.Exit(exitCode)
	}

	// Print results.
	driftCount := output.PrintCheckTable(os.Stdout, healthResults, systemdResults, m.Services)

	if len(undocumented) > 0 {
		fmt.Println()
		fmt.Println("Undocumented units:")
		for _, u := range undocumented {
			fmt.Printf("  ⚠️  %s — active, no manifest entry\n", u)
		}
		driftCount += len(undocumented)
	}

	fmt.Println()
	if driftCount == 0 {
		fmt.Println("No drift detected. All services match the manifest.")
		os.Exit(0)
	}
	fmt.Printf("Summary: %d drift detected.\n", driftCount)
	os.Exit(1)
}

// cmdHistory shows stored check history and uptime summaries.
func cmdHistory(args []string) {
	serviceID := ""
	historyPath := history.DefaultPath()
	sinceStr := "7d"
	last := 20
	raw := false

	for i := 0; i < len(args); i++ {
		if !strings.HasPrefix(args[i], "--") {
			serviceID = args[i]
			continue
		}
		switch args[i] {
		case "--since":
			if i+1 >= len(args) {
				fatal("--since requires a value (e.g. 24h, 7d, 30d)")
			}
			i++
			sinceStr = args[i]
		case "--last":
			if i+1 >= len(args) {
				fatal("--last requires a number")
			}
			i++
			fmt.Sscanf(args[i], "%d", &last)
		case "--raw":
			raw = true
		case "--history-file":
			if i+1 >= len(args) {
				fatal("--history-file requires a path")
			}
			i++
			historyPath = args[i]
		case "prune":
			cmdHistoryPrune(args[i+1:], historyPath)
			return
		default:
			fatalf("unknown flag: %s", args[i])
		}
	}

	sinceD := parseDuration(sinceStr)
	sinceTS := time.Now().Add(-sinceD).Unix()

	hdb, err := history.Open(historyPath)
	if err != nil {
		fatalf("opening history: %v", err)
	}
	defer hdb.Close()

	if serviceID == "" {
		// Summary: all services with uptime % and incident count.
		// First show any open incidents.
		open, _ := hdb.QueryOpenIncidents()
		if len(open) > 0 {
			fmt.Printf("⚠️  Currently down (%d open incident(s)):\n", len(open))
			for _, inc := range open {
				dur := time.Since(inc.StartedAt).Round(time.Minute)
				errStr := ""
				if inc.FirstError != nil {
					errStr = " — " + *inc.FirstError
				}
				fmt.Printf("  ❌ %-20s %s%s\n", inc.ServiceID, dur, errStr)
			}
			fmt.Println()
		}

		fmt.Printf("%-20s  %-12s  %-10s  %s\n", "Service", "Uptime ("+sinceStr+")", "Incidents", "Last incident")
		fmt.Println(strings.Repeat("─", 65))
		// Get distinct services from incidents.
		services, _ := hdb.ServicesSeen(sinceTS)
		for _, svc := range services {
			pct, incidents, _ := hdb.UptimePct(svc, sinceTS)
			lastInc := "—"
			incs, _ := hdb.QueryIncidents(svc, sinceTS, 1)
			if len(incs) > 0 {
				inc := incs[0]
				dur := ""
				if inc.DurationSec != nil {
					dur = fmt.Sprintf(" (%s)", (time.Duration(*inc.DurationSec) * time.Second).Round(time.Minute))
				} else {
					dur = " (ongoing)"
				}
				lastInc = inc.StartedAt.Format("Jan 02 15:04") + dur
			}
			fmt.Printf("%-20s  %-12s  %-10d  %s\n",
				svc, fmt.Sprintf("%.1f%%", pct), incidents, lastInc)
		}
		return
	}

	// Per-service incident list.
	if raw {
		fmt.Printf("%s — raw checks (last %d, since %s)\n\n", serviceID, last, sinceStr)
		checks, _ := hdb.QueryChecks(serviceID, sinceTS, last)
		for _, c := range checks {
			status := "✅ up  "
			latStr := fmt.Sprintf("%dms", derefI64(c.LatencyMS))
			if c.Status != "up" {
				status = "❌ down"
				latStr = "—"
			}
			errStr := ""
			if c.ErrorStr != nil {
				errStr = "  " + *c.ErrorStr
			}
			fmt.Printf("%s  %s  %-6s%s\n",
				c.CheckedAt.Format("2006-01-02 15:04:05"), status, latStr, errStr)
		}
		return
	}

	// Incident list.
	incidents, _ := hdb.QueryIncidents(serviceID, sinceTS, last)
	pct, incCount, _ := hdb.UptimePct(serviceID, sinceTS)
	fmt.Printf("%s — last %d incidents (since %s)\n\n", serviceID, last, sinceStr)
	if len(incidents) == 0 {
		fmt.Println("  No incidents in this window.")
	}
	for _, inc := range incidents {
		marker := "❌"
		durStr := "ongoing"
		if inc.DurationSec != nil {
			durStr = (time.Duration(*inc.DurationSec) * time.Second).Round(time.Second).String()
		}
		errStr := ""
		if inc.FirstError != nil {
			errStr = "  " + *inc.FirstError
			if inc.LastError != nil && *inc.LastError != *inc.FirstError {
				errStr += " → " + *inc.LastError
			}
		}
		fmt.Printf("  %s %s  %-10s  %d checks%s\n",
			marker, inc.StartedAt.Format("Jan 02 15:04"), durStr, inc.CheckCount, errStr)
	}
	fmt.Printf("\nUptime (%s): %.1f%%  |  %d incident(s)\n", sinceStr, pct, incCount)
}

func cmdHistoryPrune(args []string, historyPath string) {
	keepStr := "90d"
	if len(args) > 0 {
		keepStr = args[0]
	}
	dur := parseDuration(keepStr)
	keepSecs := int64(dur / time.Second)

	hdb, err := history.Open(historyPath)
	if err != nil {
		fatalf("opening history: %v", err)
	}
	defer hdb.Close()
	deleted, err := hdb.Prune(keepSecs)
	if err != nil {
		fatalf("pruning: %v", err)
	}
	fmt.Printf("Pruned %d check rows older than %s. Incidents retained.\n", deleted, keepStr)
}

func parseDuration(s string) time.Duration {
	if strings.HasSuffix(s, "d") {
		var days int
		fmt.Sscanf(s, "%d", &days)
		return time.Duration(days) * 24 * time.Hour
	}
	d, err := time.ParseDuration(s)
	if err != nil {
		return 7 * 24 * time.Hour // default 7d
	}
	return d
}

func derefI64(p *int64) int64 {
	if p == nil {
		return 0
	}
	return *p
}

// cmdValidate parses and validates the manifest without making any network calls.
// Exit 0 if valid (warnings don't block). Exit 1 on errors.
func cmdValidate(args []string) {
	manifestPath := "services.yaml"

	for i := 0; i < len(args); i++ {
		switch args[i] {
		case "--file", "-f":
			if i+1 >= len(args) {
				fatal("--file requires a path argument")
			}
			i++
			manifestPath = args[i]
		default:
			fatalf("unknown flag: %s", args[i])
		}
	}

	m, err := manifest.LoadAuto(manifestPath)
	if err != nil {
		// LoadAuto returns a pre-validated manifest; errors include parse + validation.
		// Surface them the same way validate would.
		fmt.Fprintf(os.Stderr, "Error: %v\n", err)
		os.Exit(1)
	}

	// Re-run Validate to collect warnings (LoadAuto only surfaces the first error).
	result := manifest.Validate(m)

	for _, e := range result.Errors {
		fmt.Fprintf(os.Stderr, "Error: %s\n", e)
	}
	for _, w := range result.Warnings {
		fmt.Fprintf(os.Stderr, "Warning: %s\n", w)
	}

	if !result.Valid() {
		os.Exit(1)
	}

	svcCount := len(m.Services)
	warnCount := len(result.Warnings)
	if warnCount > 0 {
		fmt.Printf("✅ Valid (%d service(s), %d warning(s))\n", svcCount, warnCount)
	} else {
		fmt.Printf("✅ Valid (%d service(s))\n", svcCount)
	}
}

// cmdReport generates a fleet uptime digest from the history database.
func cmdReport(args []string) {
	cfg := reporter.Config{
		HistoryPath: history.DefaultPath(),
		Since:       7 * 24 * time.Hour,
		Format:      "table",
	}

	for i := 0; i < len(args); i++ {
		switch args[i] {
		case "--since":
			if i+1 >= len(args) {
				fatal("--since requires a value (e.g. 24h, 7d, 30d)")
			}
			i++
			cfg.Since = parseDuration(args[i])
		case "--format":
			if i+1 >= len(args) {
				fatal("--format requires a value (table|markdown|json)")
			}
			i++
			switch args[i] {
			case "table", "markdown", "json":
				cfg.Format = args[i]
			default:
				fatalf("unknown format %q — use table, markdown, or json", args[i])
			}
		case "--webhook":
			if i+1 >= len(args) {
				fatal("--webhook requires a URL")
			}
			i++
			cfg.WebhookURL = args[i]
		case "--history-file":
			if i+1 >= len(args) {
				fatal("--history-file requires a path")
			}
			i++
			cfg.HistoryPath = args[i]
		default:
			fatalf("unknown flag: %s", args[i])
		}
	}

	r, err := reporter.Generate(cfg)
	if err != nil {
		fatalf("generating report: %v", err)
	}

	// Deliver webhook if configured.
	if cfg.WebhookURL != "" {
		if err := reporter.PostWebhook(r, cfg.WebhookURL); err != nil {
			fmt.Fprintf(os.Stderr, "⚠️  webhook failed: %v\n", err)
		} else {
			fmt.Fprintf(os.Stderr, "→ webhook delivered to %s\n", cfg.WebhookURL)
		}
	}

	// Also write to stdout (unless webhook-only was the intent and there's no TTY,
	// but for now always print — users can redirect to /dev/null).
	switch cfg.Format {
	case "markdown":
		reporter.PrintMarkdown(os.Stdout, r)
	case "json":
		type jsonRow struct {
			ID          string  `json:"id"`
			UptimePct   float64 `json:"uptime_pct"`
			Incidents   int     `json:"incidents"`
		}
		rows := make([]jsonRow, len(r.Services))
		for i, s := range r.Services {
			rows[i] = jsonRow{
				ID:        s.ID,
				UptimePct: s.UptimePct,
				Incidents: s.Incidents,
			}
		}
		out := struct {
			GeneratedAt string    `json:"generated_at"`
			Window      string    `json:"window"`
			Services    []jsonRow `json:"services"`
		}{
			GeneratedAt: r.GeneratedAt.Format(time.RFC3339),
			Window:      reporter.FormatDuration(cfg.Since),
			Services:    rows,
		}
		if err := output.WriteJSON(os.Stdout, out); err != nil {
			fatalf("json output: %v", err)
		}
	default:
		reporter.PrintTable(os.Stdout, r)
	}
}

// cmdDiff compares two manifest files and prints the schema differences.
// No network calls. Exit 0 if identical, exit 1 if differences found.
func cmdDiff(args []string) {
	quiet := false
	pathA := ""
	pathB := ""
	positional := 0

	for i := 0; i < len(args); i++ {
		switch args[i] {
		case "--quiet", "-q":
			quiet = true
		default:
			if strings.HasPrefix(args[i], "-") {
				fatalf("unknown flag: %s", args[i])
			}
			if positional == 0 {
				pathA = args[i]
			} else if positional == 1 {
				pathB = args[i]
			} else {
				fatal("svc diff takes exactly two manifest paths")
			}
			positional++
		}
	}

	if pathA == "" || pathB == "" {
		fatal("svc diff requires two manifest paths\nUsage: svc diff <old.yaml> <new.yaml>")
	}

	dataA, err := os.ReadFile(pathA)
	if err != nil {
		fatalf("reading %s: %v", pathA, err)
	}
	dataB, err := os.ReadFile(pathB)
	if err != nil {
		fatalf("reading %s: %v", pathB, err)
	}

	mA, err := manifest.ParseManifest(dataA)
	if err != nil {
		fatalf("parsing %s: %v", pathA, err)
	}
	mB, err := manifest.ParseManifest(dataB)
	if err != nil {
		fatalf("parsing %s: %v", pathB, err)
	}

	d := manifest.Diff(mA, mB)

	if d.Empty() {
		if !quiet {
			fmt.Println("No differences.")
		}
		os.Exit(0)
	}

	if !quiet {
		for _, id := range d.Added {
			fmt.Printf("Added:    %s\n", id)
		}
		for _, id := range d.Removed {
			fmt.Printf("Removed:  %s\n", id)
		}
		for _, sc := range d.Changed {
			for _, fc := range sc.Changes {
				if fc.From == "" {
					fmt.Printf("Changed:  %s — %s added (%s)\n", sc.ID, fc.Field, fc.To)
				} else if fc.To == "" {
					fmt.Printf("Changed:  %s — %s removed (was %s)\n", sc.ID, fc.Field, fc.From)
				} else {
					fmt.Printf("Changed:  %s — %s %s → %s\n", sc.ID, fc.Field, fc.From, fc.To)
				}
			}
		}
	}

	os.Exit(1)
}

func printUsage() {
	fmt.Print(`svc — Service Manifest

Usage:
  svc init              scaffold a services.yaml in the current directory
  svc status            check live health for all services
  svc check             find drift between manifest and running system
  svc validate          lint the manifest — no network calls, CI-safe
  svc diff <a> <b>      compare two manifest files — schema diff, no network calls
  svc add <id>          probe a running service and scaffold a manifest entry
  svc add --scan        probe all operator-installed systemd units at once
  svc watch             continuously poll the fleet and alert on state changes
  svc history           show per-service uptime % and incident history
  svc report            fleet uptime digest for a time window
  svc version           print version

Flags (status, check):
  --file, -f <path>     manifest file or directory (default: ./services.yaml)
                        Pass a directory to merge all *.yaml files in it
  --timeout <seconds>   health endpoint timeout (default: 5)
  --json                machine-readable JSON output

Flags (status):
  --tag <tag>           show only services with this tag

Flags (check):
  --no-systemd          skip systemd unit checks
  --no-version          skip version drift checks

Flags (validate):
  --file, -f <path>     manifest file or directory (default: ./services.yaml)

Flags (diff):
  --quiet, -q           suppress output; exit code only (0=identical, 1=diff)

Flags (init):
  --out, -o <path>      output path (default: ./services.yaml)
  --force               overwrite existing file

Flags (add):
  --port, -p <n>        port hint for health probe
  --write               append scaffolded entry to services.yaml
  --file, -f <path>     manifest file path (default: ./services.yaml)

Flags (add --scan):
  --write               append all new entries to services.yaml
  --include-known       include services already in the manifest
  --file, -f <path>     manifest file path (default: ./services.yaml)

Flags (watch):
  --webhook <url>       webhook URL for state-change alerts
  --interval <seconds>  poll interval (default: 60)
  --failures <n>        consecutive failures before DOWN alert (default: 2)

Flags (history):
  --since <duration>    window for stats (e.g. 24h, 7d, 30d; default: 7d)
  --last <n>            number of incidents to show (default: 20)
  --raw                 show raw check rows instead of incidents

Flags (report):
  --since <duration>    window for report (e.g. 24h, 7d, 30d; default: 7d)
  --format <fmt>        output format: table (default), markdown, json
  --webhook <url>       POST JSON report to this URL (also prints to stdout)
  --history-file <path> history database path

`)

}

func fatal(msg string) {
	fmt.Fprintln(os.Stderr, "Error:", msg)
	os.Exit(1)
}

func fatalf(format string, args ...any) {
	fmt.Fprintf(os.Stderr, "Error: "+format+"\n", args...)
	os.Exit(1)
}
